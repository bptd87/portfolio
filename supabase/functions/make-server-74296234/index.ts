...
// Admin Stats
app.get("/make-server-74296234/api/admin/stats", verifyAdminToken, async (c) => {
  try {
...
// ===== CATEGORIES =====
app.get("/make-server-74296234/api/categories/:type", async (c) => {
  const type = c.req.param('type');
  const { data, error } = await supabase.from('categories').select('*').eq('type', type).order('display_order');
  if (error) return c.json({ error: error.message }, 500);
  return c.json({ success: true, categories: data || [] });
});

app.get("/make-server-74296234/api/admin/categories", verifyAdminToken, async (c) => {
  const { data, error } = await supabase.from('categories').select('*').order('type, name');
  if (error) return c.json({ error: error.message }, 500);
...
app.post("/make-server-74296234/api/admin/categories/:type", verifyAdminToken, async (c) => {
  ...
  const { data, error } = await supabase.from('categories').insert({ ...category, type }).select().single();
  if (error) return c.json({ error: error.message }, 500);
