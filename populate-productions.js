 ...
 * Run this once to populate the settings API with all production data
 */
...
// Get admin token from sessionStorage (you need to be logged into admin panel first)
const adminToken = prompt("Enter your admin token from sessionStorage.getItem('admin_token'):");

if (!adminToken) {
  console.error("No admin token provided. Please log into the admin panel first.");
} else {
  ...
  fetch(`https://${projectId}.supabase.co/functions/v1/make-server-74296234/api/admin/settings`, {
    method: 'POST',
    headers: {
    ...
    .then(data => {
      console.log('✅ Production data populated successfully!', data);
      console.log(`Added ${upcomingProductions.length} upcoming productions`);
      console.log(`Added ${recentProductions.length} recent productions`);
      console.log(`Added ${assistantDesignProductions.length} assistant design productions`);
    })
    .catch(error => {
