...
import { projects as hardcodedProjects } from "../data/projects.deprecated";
...
export async function syncProjectsToDatabase() {
  ...
  try {
    ...
    const existingProjects = existingData.success
      ? existingData.projects || []
      : [];

    // Create a map for quick lookup
    const existingProjectsMap = new Map<string, any>(
    ...
    for (const project of hardcodedProjects) {
      try {
        ...
        const hasManualNotes = existing?.designNotes &&
          existing.designNotes.length > 0;
        const hasManualVideos = existing?.youtubeVideos &&
          existing.youtubeVideos.length > 0;
          ...
          galleries: hasManualGalleries
            ...
            : (project.category === "Scenic Design"
              ? {
                ...
                heroCaptions: project.image
                  ? [
                    `Production photo from ${project.title} at ${project.venue}`,
                  ]
                  : [],
                process: [],
                processCaptions: [],
          ...
          credits: project.credits
            ? (() => {
              ...
              if (project.credits.director) {
                creditsArray.push({
                  role: "Director",
                  name: project.credits.director,
                });
              }
              if (project.credits.scenicDesigner) {
                creditsArray.push({
                  role: "Scenic Designer",
                  name: project.credits.scenicDesigner,
                });
              }
              if (project.credits.coDesigner) {
                creditsArray.push({
                  role: "Co-Scenic Designer",
                  name: project.credits.coDesigner,
                });
              }
              if (project.credits.costumeDesigner) {
                creditsArray.push({
                  role: "Costume Designer",
                  name: project.credits.costumeDesigner,
                });
              }
              if (project.credits.lightingDesigner) {
                creditsArray.push({
                  role: "Lighting Designer",
                  name: project.credits.lightingDesigner,
                });
              }
              if (project.credits.soundDesigner) {
                creditsArray.push({
                  role: "Sound Designer",
                  name: project.credits.soundDesigner,
                });
              }
              if (project.credits.projectionDesigner) {
                creditsArray.push({
                  role: "Projection Designer",
                  name: project.credits.projectionDesigner,
                });
              }
              if (project.credits.choreographer) {
                creditsArray.push({
                  role: "Choreographer",
                  name: project.credits.choreographer,
                });
              }
              if (project.credits.musicalDirector) {
                creditsArray.push({
                  role: "Musical Director",
                  name: project.credits.musicalDirector,
                });
              }
              return creditsArray;
            })()
          ...
          content: project.category === "Experiential Design"
            ? [
              ...
              },
              {
                type: "text",
                content:
                  `The ${project.title} project at ${project.venue} represents an innovative approach to experiential design, blending theatrical principles with immersive installation techniques. Every element was carefully crafted to create a cohesive narrative environment.`,
              },
              ...(project.image
                ? [{
                  type: "image" as const,
                  src: project.image,
                  caption:
                    `${project.title} - ${project.venue}, ${project.year}`,
                }]
                : []),
            ]
            : undefined,
        ...
        if (response.ok) {
          results.success++;
          console.log(
            `✅ Synced: ${project.title}${
              hasManualGalleries || hasManualTags || hasManualNotes ||
                hasManualVideos
