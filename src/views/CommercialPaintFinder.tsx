...
import { MagnifyingGlass, Hash, Tag, Check, ArrowLeft, PaintBucket } from 'phosphor-react';
...
export function CommercialPaintFinder({ onNavigate }: Props) {
  ...
  // Theme Colors (Premium Fail-Safe)
  const colors = {
      ...
      // Standardized Dark Overlay for Consistency
  ...
  return (
    <>
      <SEO title="Commercial Paint Finder" description="Match hex colors to Benjamin Moore, Sherwin-Williams, and Behr paints." />
         ...
         <div className="pt-32 md:pt-40 pb-12 container mx-auto px-4 max-w-7xl">
            ...
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 mb-12">
                <div>
                    ...
                    <h1 className="font-display text-4xl md:text-5xl uppercase tracking-tight">
                        Commercial <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-500 to-red-500">Paint Finder</span>
                    </h1>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 mb-24">
                ...
                <div className="lg:col-span-8 space-y-6">
                    ...
                    >
                        <img 
                            src="/images/studio/paint-finder-abstract.webp" 
                            alt="Commercial Paint Abstract" 
                            className="w-full h-full object-cover opacity-80"
                        />
                         {/* Dark Overlay for consistent text legibility */}
                        ...
                        <div className="absolute bottom-0 left-0 right-0 p-8 flex items-end justify-between">
                            <p className="text-sm max-w-md font-medium" style={{ color: colors.headerText }}>
                                Instantly match digital colors to real-world commercial paint swatches.
                            </p>
                            <PaintBucket weight="duotone" className="w-16 h-16" style={{ color: colors.headerIcon }} />
                        </div>
                    </div>

                    {/* LIVE PREVIEW CARD */}
                    <div 
                        className="border rounded-3xl p-6 flex flex-col md:flex-row items-center gap-6"
                        style={{ 
                    ...
                    >
                        <div className="flex-1 w-full">
                            <h2 className="font-display text-lg uppercase mb-1">Target Color</h2>
                            <p className="text-xs opacity-60">Searching for closest matches to this digital value.</p>
                        </div>
                        <div className="flex items-center gap-4 w-full md:w-auto p-2 pr-6 rounded-2xl border" style={{ borderColor: colors.cardBorder, backgroundColor: colors.inputBg }}>
                            <div 
                                className="w-16 h-16 rounded-xl shadow-inner border border-white/10"
                                style={{ backgroundColor: inputColor }}
                            />
                            <div>
                                <div className="font-mono text-xl font-bold uppercase">{inputColor}</div>
                                <div className="text-[10px] font-bold uppercase tracking-wider opacity-50">Hex Value</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between mb-4 pt-4">
                        <div>
                            <h2 className="font-display text-lg uppercase">Match Results</h2>
                            <p className="text-xs opacity-60">{matches.length} matches found</p>
                        </div>
                        <MagnifyingGlass size={24} className="opacity-40" />
                    </div>
                        ...
                        <AnimatePresence>
                            {matches.map((match, idx) => (
                                <motion.div
                                    ...
                                    className="border rounded-3xl overflow-hidden group transition-all hover:scale-[1.02]"
                                    style={{ 
                                        backgroundColor: colors.cardBg, 
                                        borderColor: colors.cardBorder,
                                        boxShadow: colors.cardShadow
                                ...
                                >
                                    <div className="relative h-32 w-full" style={{ backgroundColor: match.hex }}>
                                         <div className="absolute top-3 left-3 bg-black/50 backdrop-blur text-white px-2 py-1 rounded-full text-[10px] font-bold">
                                            #{idx + 1}
                                         </div>
                                         <div className={`absolute top-3 right-3 px-3 py-1 rounded-full text-xs font-bold ${match.accuracy > 90 ? 'bg-green-500 text-white' : 'bg-black/50 text-white backdrop-blur'}`}>
                                            {match.accuracy.toFixed(0)}% Match
                                         </div>
                                    </div>
                                    
                                    <div className="p-5">
                                        <div className="text-[10px] font-bold uppercase tracking-wider opacity-60 mb-1">{match.brand}</div>
                                        <h3 className="font-display text-lg mb-4">{match.name}</h3>
                                        
                                        <div className="space-y-2 pt-3 border-t" style={{ borderColor: colors.cardBorder }}>
                                            <div className="flex justify-between items-center group/row">
                                                <span className="font-mono text-xs opacity-60">{match.code}</span>
                                                <button
                                                    onClick={() => copyToClipboard(match.code, `code-${match.id}`)}
                                                    className="text-xs flex items-center gap-1 opacity-0 group-hover/row:opacity-100 transition-opacity"
                                            ...
                                            <div className="flex justify-between items-center group/row">
                                                <span className="font-mono text-xs opacity-40">{match.hex}</span>
                                                <button
                ...
                <div className="lg:col-span-4 space-y-6">
                    ...
                    >
                        <h3 className="font-display text-lg mb-6">Input Color</h3>
                        
                        {/* Interactive Color Picker */}
                        <div 
                            className="relative w-full aspect-square rounded-2xl overflow-hidden border shadow-inner mb-6 group cursor-pointer transition-transform active:scale-95" 
                            style={{ borderColor: colors.cardBorder, backgroundColor: inputColor }}
                        >
                             <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10 opacity-0 group-hover:opacity-100 transition-opacity">
                                <span className="bg-black/50 text-white text-xs font-bold px-3 py-1 rounded-full backdrop-blur shadow-sm">Tap to Change</span>
                             </div>
                             ...
                             <input
                                ...
                                title="Choose input color"
                             />
                        </div>

                        <div className="mb-8">
                             <label htmlFor="hexCodeInput" className="block mb-2 text-xs font-bold uppercase tracking-wider opacity-60">
                                Hex Code
                            </label>
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    className="w-full px-4 py-3 pl-8 rounded-xl border font-mono uppercase focus:outline-none focus:ring-1 focus:ring-orange-500/50 transition-all text-sm"
                                    style={{ 
                        ...
                        <div className="border-t pt-6" style={{ borderColor: colors.cardBorder }}>
                            <label className="block mb-3 text-xs font-bold uppercase tracking-wider opacity-60">
                                Filter by Brand
                            </label>
                            <div className="space-y-2">
                                {brands.map(b => (
                                    <button
                                        ...
                                        className={`w-full px-4 py-3 text-left text-sm rounded-xl transition-all border ${
                                            selectedBrand === b
                                                ? 'border-orange-500 text-white' 
                                                : 'border-transparent hover:bg-black/5'
                                        }`}
                                        style={{ 
                                            backgroundColor: selectedBrand === b ? '#f97316' : 'transparent',
                                            color: selectedBrand === b ? '#ffffff' : colors.textMain
                                        }}
                                    >
                                        {b}
                                    </button>
...
export default CommercialPaintFinder;