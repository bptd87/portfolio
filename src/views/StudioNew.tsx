          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {MAIN_SECTIONS.map((section, index) => (
              <motion.div
                ...
                className={`group relative aspect-[3/4] rounded-xl overflow-hidden bg-muted ${section.route ? 'cursor-pointer' : 'cursor-default'
                  }`}
              >
                {/* Background Image */}
                <img
                  src={section.image}
                  alt={section.title}
                  className={`absolute inset-0 w-full h-full object-cover transition-transform duration-700 ${section.route ? 'group-hover:scale-105' : ''
