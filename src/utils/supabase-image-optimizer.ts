...
export function optimizeSupabaseImage(
  ...
  try {
    ...
    const { data, error } = supabase.storage.from(bucket).getPublicUrl(filePath, getPublicUrlOptions);

    if (error) {
      console.warn('Supabase getPublicUrl error:', error);
    ...
    if (data?.publicUrl) {
      ...
      if (options.focus) {
        ...
        optimizedUrl = `${optimizedUrl}${separator}focal=${options.focus.x},${options.focus.y}`;
      }
 ...
 */
export function generateBlurPlaceholder(url: string | null | undefined): string | null {
  if (!url) return null;
