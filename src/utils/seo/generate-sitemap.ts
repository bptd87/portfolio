...
export function generateSitemap(): string {
  ...
  // Portfolio filters
  const portfolioFilters = ['scenic', 'experiential', 'rendering', 'documentation'];
  ...
  // About pages
  const aboutPages = ['/news', '/cv', '/collaborators'];
  ...
  blogPosts.forEach(post => {
    urls.push({
      loc: `${SITE_URL}/scenic-insights/${post.id}`,
      lastmod: post.date,
      changefreq: 'monthly',
      priority: 0.7,
    });
