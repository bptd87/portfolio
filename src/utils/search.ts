 ...
 */
function fuzzyMatch(query: string, text: string, tolerance: number = 2): boolean {
...
export function calculateRelevance(
  ...
  } else if (fuzzyMatch(query, title, 2)) {
    score += 50;
    matchedFields.push('title');
  }
  ...
  } else if (fuzzyMatch(query, description, 2)) {
    score += 15;
    matchedFields.push('description');
  }
  ...
  } else if (keywords && fuzzyMatch(query, keywords, 2)) {
    score += 10;
    matchedFields.push('keywords');
  }
  ...
  } else if (altText && fuzzyMatch(query, altText, 2)) {
    score += 12;
    matchedFields.push('altText');
  }
  ...
  const allText = `${title} ${description} ${keywords || ''} ${altText || ''}`.toLowerCase();
