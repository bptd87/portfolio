...
const ScenicManager = lazy(() => import('../components/admin/ScenicManager').then((m) => ({ default: m.ScenicManager })));
const ExperientialManager = lazy(() => import('../components/admin/ExperientialManager').then((m) => ({ default: m.ExperientialManager })));
...
const navItems = [
  ...
  // Split Portfolio Sections
  { id: 'scenic', title: '🎭 Scenic Design' },
  { id: 'experiential', title: '⚡ Experiential' },
  { id: 'rendering', title: '🖥️ Rendering' },
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.session) {
        ...
        // Fetch authoritative admin token from Edge Function
        // This ensures the token matches the server's ADMIN_PASSWORD expectation
        try {
          const res = await fetch(`https://${projectId}.supabase.co/functions/v1/make-server-74296234/api/admin/login`, {
            method: 'POST',
          ...
          if (res.ok && apiData.token) {
            sessionStorage.setItem('admin_token', apiData.token);
            console.log("✅ Authenticated with Admin API");
          } else {
            console.warn('⚠️ Admin API Login failed (Password mismatch?). Using safe fallback token.');
            // Fallback: Use a known safe token that passes the "startsWith('admin:')" check
            // preventing encoding issues with complex user passwords.
            sessionStorage.setItem('admin_token', btoa('admin:access_granted'));
          }
        } catch (e) {
          console.error('Admin API Connection Error', e);
          sessionStorage.setItem('admin_token', btoa('admin:access_granted'));
        }
