...
import { Plus, Trash2, MoveUp, MoveDown, Type, Image as ImageIcon, Quote, List, Code, Grid, Minus, Video, Film, ChevronDown, ChevronUp, Wand2, Loader2, Upload, Bold, Italic, Link as LinkIcon, FileDown, SeparatorHorizontal } from 'lucide-react';
...
export function BlockEditor({ blocks = [], onChange }: BlockEditorProps) {
  ...
  const renderBlockPreview = (block: ContentBlock) => {
    ...
    switch (block.type) {
      case 'heading':
        const Tag = `h${block.metadata?.level || 2}` as keyof JSX.IntrinsicElements;
        return <Tag className="font-bold opacity-80">{block.content}</Tag>;
      case 'paragraph':
        return <p className="text-gray-400 line-clamp-2">{block.content}</p>;
      case 'image':
        return (
