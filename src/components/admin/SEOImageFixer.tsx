...
export function SEOImageFixer({ project, onUpdate, onAutoSave }: SEOImageFixerProps) {
    ...
    const generateCaption = async (index: number) => {
        ...
        try {
            ...
            const response = await fetch(`https://zuycsuajiuqsvopiioer.supabase.co/functions/v1/make-server-74296234/api/admin/ai/analyze-image`, {
                method: 'POST',
                headers: {
    ...
    const executeFixes = async () => {
        ...
        for (let i = 0; i < newFixes.length; i++) {
            ...
            try {
                ...
                newFixes[i] = { ...fix, status: 'error', error: err.message || 'Unknown error' };
            }
