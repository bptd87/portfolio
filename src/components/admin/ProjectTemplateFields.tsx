  ...
  const generateAltText = async (index: number, url: string) => {
    ...
    try {
      ...
      if (!key) {
        ...
        console.log('🎭 Project Context being sent:', projectContext);
        ...
        if (response.status === 401) {
          console.warn('Backend Auth Failed. Requesting local key bypass.');
          
          // Prompt user to fallback to local key
          const keyInput = window.prompt("⚠️ Admin API Connection Failed.\n\nTo continue locally, please enter your OpenAI API Key:");
          
          if (keyInput && keyInput.startsWith('sk-')) {
             setAIKey(keyInput);
             toast.success('Key saved! Retrying locally...');
             return generateAltText(index, url); // Recursively retry with new key
          }

          toast.error('Session expired. Please log in again or provide API Key.');
        ...
        // If backend fails (401 or 500), prompt for local key
        if (!response.ok || (response.status === 500)) {
          console.warn('Backend AI Failed. Requesting local key bypass.');
          
          const keyInput = window.prompt("⚠️ Backend AI unavailable.\\n\\nTo continue with local processing, please enter your OpenAI API Key:");
          
          if (keyInput && keyInput.startsWith('sk-')) {
             setAIKey(keyInput);
             toast.success('Key saved! Retrying locally...');
             return generateAltText(index, url); // Retry with new key
          }

          toast.error('Backend unavailable. Please provide API Key or try again later.');
          throw new Error(data.error || 'Backend AI failed');
        }
        
        if (data.success && data.result) {
          updateAltText(index, data.result);
